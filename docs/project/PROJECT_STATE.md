...
- **Last completed workflow session:** Session 29
...
- **Latest completed workflow artifact:** `docs/implementation/ACCOUNT_MANAGEMENT_REGISTER.md`
...
- **Completed implementation slices:** `SLICE-FOUND-001`, `SLICE-FOUND-002`, `SLICE-IAM-001`, `SLICE-IAM-002`, `SLICE-ACC-001`
- **Next approved implementation slice:** `SLICE-UI-001` (Product UI Foundation & Existing Screen Retrofit) — per the 2026-08-06 roadmap-execution amendment to `docs/implementation/MVP_IMPLEMENTATION_PLAN.md`
- **Next recommended task:** Session 30 — `SLICE-UI-001`: Product UI Foundation & Existing Screen Retrofit. `SLICE-ACC-002` and `SLICE-CAT-001` remain not started and form the next coordinated wave after `SLICE-UI-001` (Wave 1)
...
Session 28 — Identity, Onboarding, and Workspace Isolation is complete. `SLICE-IAM-002` was implemented, reviewed, committed, pushed, and merged into `dev`. Pull Request CI and post-merge GitHub Actions are green. Exactly four implementation slices had completed at that point. `main` remains production-only and has not received these implementation slices.

Session 29 — Account Management Baseline is complete. `SLICE-ACC-001` was implemented, reviewed through multiple correction passes, validated locally, committed, pushed, and merged into `dev`. Its Pull Request GitHub Actions and the post-merge `dev` GitHub Actions run are green. Exactly five implementation slices have completed. `main` remains production-only and has not received these implementation slices.

A roadmap-execution amendment was approved by the product owner on 2026-08-06 and recorded in `docs/implementation/MVP_IMPLEMENTATION_PLAN.md`'s Roadmap Execution Amendment appendix, `docs/architecture/ARCHITECTURE_BASELINE.md`'s Frontend Architecture clarification, `CLAUDE.md`, and this document. It introduces `SLICE-UI-001` ("Product UI Foundation & Existing Screen Retrofit") as the next approved implementation slice, ahead of `SLICE-ACC-002`, and establishes a coordinated execution-wave model, a risk-based validation strategy, and a canonical frontend tooling-governance policy (Section 5 and Section 17 below). The amendment is documentation only — it retains every existing slice identifier and its original acceptance criteria unchanged, does not weaken any mandatory CI, financial-integrity, security, isolation, or database guarantee, and does not itself start Session 30, `SLICE-UI-001`, `SLICE-ACC-002`, `SLICE-CAT-001`, or any later slice.

Session 30 — `SLICE-UI-001` ("Product UI Foundation & Existing Screen Retrofit") is next, and is the only next approved implementation slice. `SLICE-UI-001` and every later slice remain not started. `SLICE-ACC-002` and `SLICE-CAT-001` remain not started and form the next coordinated wave after `SLICE-UI-001`; they are not technically blocked by it (their own dependencies, `SLICE-ACC-001` and `SLICE-IAM-002` respectively, are unchanged). Product, domain, Architecture, and implementation planning may only be reopened through explicit review when a genuine conflict or blocking defect is discovered; implementation may not silently reinterpret them.
...
- Implementation: current workflow stage. Sessions 25–29, covering `SLICE-FOUND-001`, `SLICE-FOUND-002`, `SLICE-IAM-001`, `SLICE-IAM-002`, and `SLICE-ACC-001`, are complete, reviewed, and merged to `dev`; exactly five implementation slices have completed. Session 30 / `SLICE-UI-001` is next and has not started (2026-08-06 roadmap-execution amendment). `SLICE-ACC-002` and `SLICE-CAT-001` form the next coordinated wave after `SLICE-UI-001`. Every later slice remains not started.
- Review and Testing: slice-level review and testing are complete for `SLICE-FOUND-001`, `SLICE-FOUND-002`, `SLICE-IAM-001`, `SLICE-IAM-002`, and `SLICE-ACC-001`; the later workflow-wide Review and Testing stages have not started. Release has not started. Managed identity, application-session persistence, Private Beta entitlement redemption, atomic onboarding, one private Workspace and starter Account per User, the Isolation Gate, and the Account management baseline (create, list, rename, archive, restore, delete-eligibility evaluation) now exist; Opening Balance/Effective Date correction, Category, financial behavior, and deployment have not started.

Completed workflow artifacts and implementation outcomes, in order: Product Identity; MVP PRD; Ubiquitous Language; Domain Concept Model; Domain Object Candidates; Aggregate Candidate Analysis; Domain Behavior Analysis; Domain Behavior Decision Tables and Boundary Participation Analysis; Domain Decision Resolution; Executable Domain Specification; Architecture Baseline; MVP Implementation Plan; `SLICE-FOUND-001` technical foundation and its Foundation Dependency Register; `SLICE-FOUND-002` database foundation and its Database Foundation Register; `SLICE-IAM-001` managed identity and persisted application-session implementation and its Identity and Session Evidence Register; `SLICE-IAM-002` Private Beta onboarding and Workspace-isolation implementation, its Onboarding and Isolation Register, and its IAM-002 Onboarding Testing Guide; `SLICE-ACC-001` Account management implementation (create, list, rename, archive, restore, delete-eligibility evaluation) and its Account Management Register.
...
**Current gate status:** the full Foundation Gate is complete; the `SLICE-IAM-001` identity/session baseline and acceptance evidence are complete; the Isolation Gate is complete; and the `SLICE-ACC-001` Account-management baseline and acceptance evidence are complete. The First Financial Write Gate, Cross-Boundary Consistency Gate, Correction/Recalculation Gate, Reporting Consistency Gate, Pre-Beta Security Gate, Recovery Gate, and Private Beta Acceptance Gate remain not started. `MVP_IMPLEMENTATION_PLAN.md` defines no separate formal "Account Management Gate" — Account-management completion is recorded as slice-level acceptance evidence (INV-ACC-01–03 proven under property tests), not as one of the nine named gates. `SLICE-CAT-001` (Category) remains dependency-unblocked but not selected. Per the 2026-08-06 roadmap-execution amendment, `SLICE-UI-001` (Product UI Foundation & Existing Screen Retrofit) is the next selected slice; `SLICE-ACC-002` and `SLICE-CAT-001` form the next coordinated wave after it. The Account-management baseline is complete, but financial-write readiness is not complete.
...
### Completed Account Management Baseline (Session 29 / `SLICE-ACC-001`)

`SLICE-ACC-001` was implemented, reviewed through two further correction passes on the same working tree, validated locally, committed, pushed, and merged into `dev`. Its Pull Request GitHub Actions and the post-merge `dev` GitHub Actions run are green. The reviewed evidence and delivered baseline are recorded in `docs/implementation/ACCOUNT_MANAGEMENT_REGISTER.md`.

The delivered baseline includes:

- authenticated Users listing only Accounts belonging to their own private Workspace, with the IAM-002 starter Account managed as an ordinary Account;
- creation of additional Accounts accepting name, a fixed Account Type, a whole-Rupiah non-negative Opening Balance, and a date-only Opening-Balance Effective Date, with duplicate names accepted; creation establishes stable identity, Active lifecycle, Total Account Balance = Unallocated Amount = Opening Balance, version 1, and no Financial Event/Income/Category/Fund/Debt behavior;
- Account rename preserving identity, Workspace, Account Type, Opening Balance, Effective Date, both balances, and lifecycle state, with duplicate names remaining valid and rename never creating a new Account;
- archive permitted only from Active lifecycle with Total Account Balance exactly Rp0, checked atomically and enforced independently by a permanent PostgreSQL CHECK constraint (`accounts_archived_at_consistency`) in addition to the application's own conditional `UPDATE`; restore reactivates the same Account row; no money is moved, corrected, spent, or deleted by archive/restore; archived Accounts remain visible and retained;
- delete-eligibility evaluation only, with no deletion performed and no Account DELETE endpoint: `eligible = (Opening Balance = Rp0) AND no Financial Event history AND no other persisted dependency`. The dependency check is real, not vacuous — it queries the existing IAM-002 `onboarding_idempotency.account_id` (`ON DELETE RESTRICT`) dependency, so the starter Account currently evaluates as **not eligible** (`DEPENDENCY_EXISTS`) even though its Opening Balance is Rp0, while an additional dependency-free zero-opening Account may be eligible; a non-zero-balance Account is ineligible for its Opening Balance alone. Eligibility evaluation is read-only and is not authorization to delete;
- optimistic concurrency via a positive PostgreSQL BIGINT `version`, represented as a decimal string over HTTP; rename, archive, and restore require `expectedVersion`; an accepted mutation increments version; a stale mutation returns a safe `ACCOUNT_CONFLICT` and leaves prior confirmed state unchanged; no process-local lock is used;
- strict UUID validation for Account IDs and bounded-BIGINT validation for Account versions, rejected before either reaches PostgreSQL; Account Type remains immutable; Opening Balance and Opening-Balance Effective Date remain immutable in this slice; money remains whole-Rupiah exact strings over HTTP; date-only values do not shift through timezone conversion.

**Delivered database boundary** (`database/migrations/0003_account_management.sql`, extending the existing `accounts` table rather than replacing it):

- lifecycle states currently implemented are `active` and `archived` only; fields added include lifecycle metadata (`archived_at`), an `updated_at` timestamp, and an optimistic `version bigint` (`>= 1`);
- the inherited IAM-002 constraints that permanently pinned Total and Unallocated to Opening Balance were removed so a later Fund Allocation slice is not blocked; existing non-negative constraints remain; the permanent constraint equivalent to `unallocated_balance <= total_balance` is enforced; every Archived Account must have `total_balance = 0`, enforced by a permanent CHECK, not only by application logic;
- Account RLS is split by database command into explicit SELECT/INSERT/UPDATE policies; there is no DELETE policy; the INSERT policy enforces creation facts (Total = Opening, Unallocated = Opening, Active, `archived_at` null, `version = 1`) as a database-level `WITH CHECK`;
- forced PostgreSQL RLS remains enabled; the application role is a non-owner with no `BYPASSRLS`, has restricted column-level mutation rights (UPDATE is granted only on `name`, `lifecycle_status`, `archived_at`, `updated_at`, `version` — never on `opening_balance`, `total_balance`, `unallocated_balance`, `type`, or the effective date), and has no DELETE privilege;
- Workspace authority remains server-derived through the existing chain: opaque application session → local User → owned Workspace → transaction-local `app.user_id`/`app.workspace_id`.

**Delivered evidence** (`docs/implementation/ACCOUNT_MANAGEMENT_REGISTER.md`): Account contract tests; unit and property-oriented Account tests, including INV-ACC-01, INV-ACC-02, and the zero-allocation form of INV-ACC-03, plus a generated fast-check property sequence over chained lifecycle/action steps (rename, archive, restore, delete-eligibility evaluation, stale-version attempts); PostgreSQL migration tests, including a real IAM-002→ACC-001 upgrade-preservation test; Account RLS and Workspace-isolation tests, including cross-Workspace denial and a forced single-physical-connection concurrency proof; concurrency tests; API and security integration tests; frontend interaction tests; a dedicated `account-management` CI job; and full aggregate CI, production build, and dependency audit continuing to pass. The final reviewed Account-specific automated total recorded is **124 tests**: 23 contract, 56 unit/property (API), 23 PostgreSQL integration, 11 API/security integration, and 11 frontend — this breakdown was independently re-verified against the current test commands during this synchronization. Separately — and not counted among those 124 automated tests — a manual database aggregate integrity validation (part of the 2026-08-06 manual operator live validation recorded below) found no invalid archived Account and no invalid balance Account.

**Live validation status:** `ACCOUNT_MANAGEMENT_REGISTER.md` records a dated manual operator live validation completed on 2026-08-06, distinct from the automated test evidence and from GitHub Actions CI evidence. The manual operator live validation exercised the local Next.js web application and local NestJS/Fastify API against the local PostgreSQL environment and the existing Clerk development instance, covering starter-Account display, rename persistence, additional-Account creation, zero/positive-opening creation, duplicate names, zero-balance archive/restore, blocked non-zero archive, starter-Account and additional-Account delete-eligibility outcomes, the absence of any delete action, and the absence of any Account Type/Opening Balance/Effective Date correction control. A safe database aggregate validation query found no invalid archived Account and no invalid balance Account. The final merged ACC-001 baseline therefore includes three distinct, non-overlapping evidence categories: automated test evidence (124 Account-specific tests, unchanged), manual operator live validation evidence (2026-08-06, recorded above), and GitHub Actions CI evidence (Pull Request checks and the post-merge `dev` run, both green). `ACCOUNT_MANAGEMENT_REGISTER.md` is the authoritative source for the complete manual-validation outcome list; this document summarizes it and must not be treated as a substitute for reading the register directly.

`SLICE-ACC-001` explicitly does not implement Opening Balance correction; Opening-Balance Effective Date correction; Impact Preview; recalculation; Account Type change; negative balances; credit cards; overdraft; liability Accounts; Category; Dedicated Fund; Debt Record; Financial Event; Income; Expense; Transfer; Fund Allocation; Fund Release; Debt Repayment; dashboard; reporting; permanent Account deletion; `SLICE-ACC-002`; `SLICE-ACC-003`; `SLICE-CAT-001`; or any later slice. Delete eligibility means evaluating whether deletion would be allowed by the approved rules; it does not authorize physical or permanent deletion.

### Next Approved Slice: Product UI Foundation (Session 30 / `SLICE-UI-001`)

The product owner approved a roadmap-execution amendment on 2026-08-06 (recorded in full in `docs/implementation/MVP_IMPLEMENTATION_PLAN.md`'s Roadmap Execution Amendment appendix). It makes `SLICE-UI-001` — **"Product UI Foundation & Existing Screen Retrofit"** — the next approved implementation slice, inserted ahead of `SLICE-ACC-002` in execution order. `SLICE-UI-001` is the only next approved implementation slice and remains not started. Its dependencies, `SLICE-IAM-002` and `SLICE-ACC-001`, are both merged, so its entry condition is satisfied.

Its bounded scope (full detail in `docs/implementation/MVP_IMPLEMENTATION_PLAN.md`'s Roadmap Execution Amendment and `docs/implementation/UI_FOUNDATION_REGISTER.md`): design tokens; typography, spacing, semantic-color, border/radius/shadow policy; a reusable application shell with desktop and mobile navigation, placed at a location selected during implementation rather than predetermined; shared form/button/alert/dialog patterns; loading, skeleton, empty, error, success, and disabled(-with-reason) states; responsive behavior; keyboard accessibility; visible focus; reduced-motion behavior; and retrofit of the existing login, signup, and forgot-password screens (each confirmed by direct inspection to be an application-rendered React/shadcn form calling a Clerk hook only for the identity operation, not a Clerk-hosted UI surface), onboarding, already-onboarded state, accounts list, account create/rename/archive/restore, and delete-eligibility presentation — without changing their approved backend/domain behavior. The existing global root layout (`apps/web/app/layout.tsx`) wraps every route including the public auth screens and is **not** predetermined as the authenticated-shell placement (`UI_FOUNDATION_REGISTER.md` §8).

It explicitly excludes: new financial behavior; `SLICE-ACC-002` correction behavior; Category behavior; Income/Expense/Transfer/Fund/Debt/Correction/Reporting/Security feature implementation; API contract changes beyond a separately justified existing-UI-defect repair; database migration; domain-rule change; rewriting IAM/onboarding/Account behavior; permanent deletion; broad dashboard implementation; speculative post-MVP design; dependency installation without explicit approval; and forced use of every frontend tool on every screen.

Its exit criteria: one coherent design-token baseline exists; existing user-facing screens use the shared foundation; no known horizontal overflow at the minimum supported mobile width; the existing validated IAM, onboarding, isolation, and Account behavior remains unchanged; no authoritative financial calculation moves to the browser; source/provenance and dependency review are complete for any external pattern used; UI evidence is recorded in `docs/implementation/UI_FOUNDATION_REGISTER.md`; and no later feature slice is silently started.

`SLICE-UI-001` must inspect the existing frontend implementation and component baseline (`apps/web/components.json`, `apps/web/package.json`, `apps/web/app/styles.css`, `apps/web/components/`, `apps/web/app/`) before adding any component or dependency. shadcn/ui with the existing Base UI primitive baseline is canonical (Section 5's Frontend-tool navigation, updated below). No UI/UX Pro Max skill is repository-locked (`skills-lock.json`, `.agents/skills/`, `.claude/skills/` inspected); no `motion`/`framer-motion` dependency exists in `apps/web/package.json`. 21st.dev is an external component/pattern registry, not a package or skill that gets "installed" — no persistent registry configuration or documented 21st-derived component provenance was found in this repository. None of the three may be claimed as installed, locked, or approved for use beyond what `UI_FOUNDATION_REGISTER.md` §5 records; further verification or installation, if needed, is a `SLICE-UI-001` preflight activity, not something this documentation amendment performs.

### Following Coordinated Wave — Account Correction Slice (`SLICE-ACC-002`, not yet next)

`SLICE-ACC-002` and `SLICE-CAT-001` form Wave 1, the coordinated wave immediately after `SLICE-UI-001` (Roadmap Execution Amendment). They remain not started and are not technically blocked by `SLICE-UI-001` — `SLICE-ACC-002`'s own dependency (`SLICE-ACC-001`, merged) is unchanged. `SLICE-ACC-002`'s approved scope, recorded below, is unchanged by this amendment.

Per `docs/implementation/MVP_IMPLEMENTATION_PLAN.md`'s vertical-slice table, `SLICE-ACC-002` is titled **"Early Account correction: opening balance / effective-date correction when no dependent Financial Event history exists yet"** (Behavior IDs: AC-06/07, no-history case only; Architecture Decisions Applied: ARCH-RECALC-01, DEC-LIFE-05). Its entry condition — `SLICE-ACC-001` merged — is satisfied, but it is not the next slice to start; `SLICE-UI-001` is.

Its bounded scope, exactly as recorded in the plan:

- Backend: Account correction plus a recalculation harness scoped to the no-dependent-history case;
- Frontend: a correction flow with Impact Preview;
- Persistence: none beyond `SLICE-ACC-001`;
- Test/evidence scope: unit, property, and recalculation.

Its explicit exclusions, exactly as recorded in the plan: correction against existing Income/Expense/other Event history (that is `SLICE-ACC-003`); cross-concept recalculation generalized across all six event types (that is `SLICE-CORR-002`).

Its exit criterion, exactly as recorded in the plan: an EXAMPLE-28-equivalent blocked-correction test passes for the no-history case, and validation, the Impact Preview trigger, and an atomic accepted update are all proven. Recalculation is transactional; a blocked proposal leaves prior state (the slice's rollback/recovery basis). Commit boundary: one PR.

At minimum, the following boundaries already established remain in force: Account Type is not editable — `SLICE-ACC-002`'s approved scope is opening balance/effective-date correction only and does not reopen Account Type; `SLICE-ACC-001` does not silently authorize Opening Balance or Effective Date changes on its own, and only `SLICE-ACC-002`'s own approved scope authorizes the no-history correction case; no later Account, Category, Fund, Debt, Financial Event, dashboard, or reporting behavior may be pulled into `SLICE-ACC-002` beyond what is named above; no financial behavior is marked complete merely because Account management exists; permanent Account deletion remains unimplemented and is not assigned to `SLICE-ACC-002`.

`SLICE-ACC-003` (history-aware correction against existing later Income/Expense history) and every later slice remain not started and are not authorized by this synchronization.

**Frontend-tool navigation (updated 2026-08-06):** the canonical frontend tooling hierarchy, in order, is (1) product principles and domain constraints, (2) Annotasi Finance design tokens and interaction standards, (3) shadcn/ui with the existing Base UI primitive baseline, (4) UI/UX Pro Max for design reasoning/exploration/review, (5) 21st.dev for selective pattern discovery, (6) Motion for purposeful animation — see `CLAUDE.md` §7 and `docs/implementation/UI_FOUNDATION_REGISTER.md` §5 for full governance. shadcn/ui with Base UI was adopted in `SLICE-IAM-001` (`apps/web/components.json`: style `base-nova`, `@base-ui/react`) and remains the approved component baseline; Annotasi Finance owns and reviews the resulting component source, and `SLICE-UI-001` must not reinitialize shadcn or introduce a competing primitive library. No UI/UX Pro Max skill is repository-locked, and no `motion`/`framer-motion` dependency exists in `apps/web/package.json` (`skills-lock.json`, `.agents/skills/`, `.claude/skills/`, `apps/web/package.json` inspected). 21st.dev is an external component/pattern registry and source-discovery channel, not a package or repository skill — its absence from `package.json`/`skills-lock.json` does not by itself prove no existing source has ever originated from it, and any component sourced from it must still pass the External Component Intake Policy (`UI_FOUNDATION_REGISTER.md` §12) regardless of whether using it results in an installed dependency. None of the three tools may be claimed as installed, locked, or approved for use beyond `UI_FOUNDATION_REGISTER.md` §5's record; further verification or installation, if needed, is a `SLICE-UI-001` preflight activity, not performed by this documentation amendment. None of these tools may change product, domain, Architecture, identity, authorization, session, onboarding, or isolation rules, and none may introduce domain logic or financial authority into the browser.
...
- **Account Management Baseline (Session 29 / `SLICE-ACC-001`):** Session 29 is complete. Its required pre-implementation full reads were `CLAUDE.md`, `docs/project/PROJECT_STATE.md`, `docs/implementation/MVP_IMPLEMENTATION_PLAN.md`, `docs/architecture/ARCHITECTURE_BASELINE.md`, `docs/implementation/DATABASE_FOUNDATION_REGISTER.md`, `docs/implementation/IDENTITY_SESSION_REGISTER.md`, and `docs/implementation/ONBOARDING_ISOLATION_REGISTER.md`, supplemented by targeted authoritative product/domain sections for Account definition, Account Types, Account lifecycle, non-negative balance, archive eligibility, restore behavior, deletion eligibility, duplicate Account names, Workspace isolation, and Opening Balance as starting state, and by inspection of the Account schema delivered by `SLICE-IAM-002` before migration planning. The evidence produced and finalized by Session 29 is recorded in `docs/implementation/ACCOUNT_MANAGEMENT_REGISTER.md`, together with the Account migration/runtime/API/web implementation and the `account-management` CI job. This entry preserves the completed session's source-loading baseline and separately identifies its outputs; `ACCOUNT_MANAGEMENT_REGISTER.md` was not a pre-implementation input to Session 29 — it did not yet exist and was produced by Session 29.
- **Product UI Foundation (Session 30 / `SLICE-UI-001`):** Read completely: `CLAUDE.md`, `docs/project/PROJECT_STATE.md`, `docs/implementation/MVP_IMPLEMENTATION_PLAN.md` (including the Roadmap Execution Amendment appendix), `docs/architecture/ARCHITECTURE_BASELINE.md` (including its Frontend Architecture clarification), `docs/implementation/UI_FOUNDATION_REGISTER.md`, `docs/product/PRODUCT_IDENTITY.md`, and `docs/implementation/DATABASE_FOUNDATION_REGISTER.md`, `docs/implementation/IDENTITY_SESSION_REGISTER.md`, `docs/implementation/ONBOARDING_ISOLATION_REGISTER.md`, and `docs/implementation/ACCOUNT_MANAGEMENT_REGISTER.md` as the implementation registers for the screens being retrofitted. Read targeted `docs/product/ANNOTASI_FINANCE_MVP_PRD.md` sections for mobile-first requirements (§21), Bahasa Indonesia UI language (§22), onboarding (§10), and account-management experience (§12). Inspect the existing frontend implementation and component baseline (`apps/web/components.json`, `apps/web/package.json`, `apps/web/app/styles.css`, `apps/web/components/`, `apps/web/app/`) directly before adding any component or dependency — `SLICE-UI-001` must not reinitialize shadcn or assume a documentation-only starting point. This entry is the next approved slice's source-loading baseline; Session 30 and `SLICE-UI-001` begin only through explicit instruction and remain not started.
- **Account Correction Baseline (Wave 1, following `SLICE-UI-001` — not Session 30):** Read completely: `CLAUDE.md`, `docs/project/PROJECT_STATE.md`, `docs/implementation/MVP_IMPLEMENTATION_PLAN.md`, `docs/architecture/ARCHITECTURE_BASELINE.md`, `docs/implementation/DATABASE_FOUNDATION_REGISTER.md`, `docs/implementation/IDENTITY_SESSION_REGISTER.md`, `docs/implementation/ONBOARDING_ISOLATION_REGISTER.md`, and `docs/implementation/ACCOUNT_MANAGEMENT_REGISTER.md`. Read targeted authoritative product/domain sections for AC-06/AC-07 opening balance and effective-date correction, the no-dependent-history case boundary, Impact Preview triggering (DEC-LIFE-05), and the exclusions belonging to `SLICE-ACC-003` and `SLICE-CORR-002`. Inspect the current Account schema and the delivered `0003_account_management.sql` migration before planning any new migration. `SLICE-ACC-002`'s scope is unchanged by the 2026-08-06 amendment; only its execution order relative to `SLICE-UI-001` changed (it is Wave 1, not Session 30). It begins only through explicit instruction and remains not started.
...
| Account Management Baseline — `SLICE-ACC-001` | `CLAUDE.md`, `PROJECT_STATE.md`, `MVP_IMPLEMENTATION_PLAN.md`, `ARCHITECTURE_BASELINE.md`, `DATABASE_FOUNDATION_REGISTER.md`, `IDENTITY_SESSION_REGISTER.md`, `ONBOARDING_ISOLATION_REGISTER.md` | Targeted authoritative Account sections covering definition, Types, lifecycle, non-negative balance, archive/restore/delete eligibility, duplicate names, Workspace isolation, Opening Balance as state, and ACC-002 exclusions; inspection of the IAM-002 Account schema before migration planning; `ACCOUNT_MANAGEMENT_REGISTER.md` as delivered evidence | No — targeted only | **Completed in Session 29.** Created/listed/renamed/archived/restored Accounts and evaluated deletion eligibility with Account management UI, Workspace-scoped persistence/RLS, and unit/property/PostgreSQL/API-security/frontend/isolation evidence (124 Account-specific tests). It excludes opening-state correction, Impact Preview, recalculation, permanent deletion, every financial behavior, `SLICE-ACC-002`, `SLICE-CAT-001`, and later slices. `ACCOUNT_MANAGEMENT_REGISTER.md` is the reviewed evidence source. |
| Product UI Foundation — `SLICE-UI-001` | `CLAUDE.md`, `PROJECT_STATE.md`, `MVP_IMPLEMENTATION_PLAN.md` (incl. Roadmap Execution Amendment), `ARCHITECTURE_BASELINE.md` (incl. Frontend Architecture clarification), `UI_FOUNDATION_REGISTER.md` | Product identity (`PRODUCT_IDENTITY.md`); targeted PRD frontend sections (§10, §12, §21, §22); `IDENTITY_SESSION_REGISTER.md`, `ONBOARDING_ISOLATION_REGISTER.md`, `ACCOUNT_MANAGEMENT_REGISTER.md` as the registers for the screens being retrofitted; direct inspection of the existing frontend implementation and component baseline (`apps/web/components.json`, `apps/web/package.json`, `apps/web/app/styles.css`, `apps/web/components/`, `apps/web/app/`) before adding any component or dependency | No — targeted only | **Next approved slice (Session 30); not started.** Product UI Foundation & Existing Screen Retrofit: design tokens, application shell, navigation, shared interaction states, and retrofit of existing auth/onboarding/Account screens, without changing their approved backend/domain behavior. Excludes all new financial behavior, `SLICE-ACC-002`, Category, and every later feature slice. |
| Account Correction Baseline — `SLICE-ACC-002` | `CLAUDE.md`, `PROJECT_STATE.md`, `MVP_IMPLEMENTATION_PLAN.md`, `ARCHITECTURE_BASELINE.md`, `DATABASE_FOUNDATION_REGISTER.md`, `IDENTITY_SESSION_REGISTER.md`, `ONBOARDING_ISOLATION_REGISTER.md`, `ACCOUNT_MANAGEMENT_REGISTER.md` | Targeted authoritative sections for AC-06/AC-07 (no-history case), DEC-LIFE-05 Impact Preview triggering, and the `SLICE-ACC-003`/`SLICE-CORR-002` exclusions; inspection of the current Account schema and `0003_account_management.sql` before any new migration | No — targeted only | **Wave 1 (following `SLICE-UI-001`); not started; not the next slice to start.** Early Account correction: opening balance / effective-date correction when no dependent Financial Event history exists yet, with a recalculation harness scoped to the no-dependent-history case and a correction flow with Impact Preview. Scope unchanged by the 2026-08-06 amendment. It excludes correction against existing Income/Expense/other Event history (`SLICE-ACC-003`) and cross-concept recalculation generalized across all six event types (`SLICE-CORR-002`). |
...
| Coding | `CLAUDE.md`, `PROJECT_STATE.md`, `MVP_IMPLEMENTATION_PLAN.md`, `ARCHITECTURE_BASELINE.md` | Sources required by the selected slice | N/A | **Current workflow stage.** Exactly five slices, `SLICE-FOUND-001`, `SLICE-FOUND-002`, `SLICE-IAM-001`, `SLICE-IAM-002`, and `SLICE-ACC-001`, are complete. `SLICE-UI-001` is the only next approved slice (2026-08-06 roadmap-execution amendment) and remains not started; `SLICE-ACC-002` and `SLICE-CAT-001` form the next coordinated wave after it; no later slice is authorized. |
| Code review | `CLAUDE.md`, `PROJECT_STATE.md` | Relevant specification, plan, Architecture, and implementation evidence | N/A | Slice-level implementation review is complete for `SLICE-FOUND-001`, `SLICE-FOUND-002`, `SLICE-IAM-001`, `SLICE-IAM-002`, and `SLICE-ACC-001`; later reviews occur only for explicitly started slices. |
...
- Session 29 is the last completed implementation session; `SLICE-FOUND-001`, `SLICE-FOUND-002`, `SLICE-IAM-001`, `SLICE-IAM-002`, and `SLICE-ACC-001` are reviewed, complete, and merged into `dev`. Exactly five implementation slices are complete.
- Implementation remains the current workflow stage.
- `SLICE-ACC-001` was implemented, reviewed through multiple correction passes, validated locally, committed, pushed, and merged into `dev`. Its Pull Request GitHub Actions and the post-merge `dev` GitHub Actions run are green. Its manual operator live-validation evidence (2026-08-06) is merged into `dev` and recorded in `docs/implementation/ACCOUNT_MANAGEMENT_REGISTER.md`.
- The full Foundation Gate, the Isolation Gate, the IAM-001 identity/session baseline and acceptance evidence, and the ACC-001 Account-management baseline and acceptance evidence are complete. The Account-management baseline (create, list, rename, archive, restore, and delete-eligibility evaluation, with Workspace-scoped persistence/RLS and the evidence named in Section 5) is complete.
- `main` remains production-only and has received none of the five implementation slices, unless a later synchronization records otherwise.
- The current branch (`docs/accelerate-roadmap-and-ui-foundation` at the time of this amendment) performs a **documentation-only roadmap-execution amendment**. No implementation slice is being started by this amendment.
- The product owner approved the roadmap-execution amendment on 2026-08-06. Session 30 is now reserved for **`SLICE-UI-001` — Product UI Foundation & Existing Screen Retrofit**, per `MVP_IMPLEMENTATION_PLAN.md`'s Roadmap Execution Amendment appendix, not `SLICE-ACC-002`.
- `SLICE-UI-001` is the only next approved slice and has not started.
- `SLICE-ACC-002` and `SLICE-CAT-001` remain not started and form the next coordinated feature wave (Wave 1) after `SLICE-UI-001`; they are not technically blocked by `SLICE-UI-001`. `SLICE-ACC-003` and every later slice remain not started. `SLICE-SEC-002` remains eligible for parallel operational execution (its dependency on `SLICE-FOUND-002` is already satisfied).
- The First Financial Write Gate, Cross-Boundary Consistency Gate, Correction/Recalculation Gate, Reporting Consistency Gate, Pre-Beta Security Gate, Recovery Gate, and Private Beta Acceptance Gate remain not started; financial-event write readiness remains incomplete.
- Session 30, `SLICE-UI-001`, `SLICE-ACC-002`, `SLICE-CAT-001`, `SLICE-SEC-002`, and every later implementation slice begin only through explicit instruction. This documentation-only amendment does not begin any of them or any implementation work.
...
### `docs/implementation/ACCOUNT_MANAGEMENT_REGISTER.md`
- **Authority role:** Reviewed evidence register for Account create/list/rename/archive/restore/delete-eligibility behavior, the extended `accounts` database boundary, RLS/concurrency/validation evidence, and the live-validation status delivered by Session 29 / `SLICE-ACC-001`.
- **When to read:** Before broader Account persistence, correction, or lifecycle work (including `SLICE-ACC-002`); before changing Account RLS, concurrency, validation, or database grants; and before claims about the behavior or test evidence delivered by `SLICE-ACC-001`.
- **Must not be replaced by:** `PROJECT_STATE.md` navigation metadata, product/domain authority, `MVP_IMPLEMENTATION_PLAN.md`'s slice boundary, or assumptions inferred from the current UI. It is not product/domain authority itself and must not be silently modified by later sessions.

### `docs/implementation/UI_FOUNDATION_REGISTER.md`
- **Authority role:** Implementation-level UI governance register for the approved `SLICE-UI-001` (Product UI Foundation & Existing Screen Retrofit): product experience direction, the repository-proven existing technical baseline, the canonical frontend tooling hierarchy, component architecture, design-token scope, application shell, existing-screen retrofit inventory, required interaction states, responsive/accessibility baseline, external-component intake policy, validation strategy, and acceptance criteria. Created by the 2026-08-06 roadmap-execution amendment; `SLICE-UI-001` remains not started as of its creation.
- **When to read:** Before any `SLICE-UI-001` work; before adding, generating, or adapting a frontend component or dependency; before changing design tokens, the application shell, or navigation; and before any claim about which frontend tools are installed or approved.
- **Must not be replaced by:** Product/domain authority, `ARCHITECTURE_BASELINE.md`'s Frontend Architecture section (which it implements, not overrides), or `MVP_IMPLEMENTATION_PLAN.md`'s slice boundary. It carries no product or domain authority itself and must not be silently modified by later sessions.
