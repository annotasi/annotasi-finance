...
**Frontend component-foundation clarification (2026-08-06 roadmap-execution amendment — additive; does not reopen or weaken the decisions above).** shadcn/ui source-controlled components, generated into the repository and reviewed like any other source, are the canonical component foundation implementing ARCH-FRONT-01; Base UI remains the selected underlying primitive baseline. Tailwind CSS variables and the project's design tokens define the shared visual system that page-specific styling must build on rather than duplicate. External component registries, marketplace pattern sources, and design-assistant tooling (e.g., a selective pattern accelerator or a design-review assistant) are **inputs to** this architecture, never an authority over it: any component adapted from such a source must be normalized into the local component architecture (tokens, primitives, accessibility) before it is treated as part of the product. Purposeful animation tooling is optional and non-authoritative — it may improve perceived understanding of a state change but never gates, delays, or substitutes for a server-confirmed financial outcome, and must respect a reduced-motion preference. Consistent with this baseline's existing browser-authority constraints (this section and Section 6), the browser remains an untrusted presentation boundary: no animation library, component registry, or external design tool may introduce domain logic, financial calculation, or financial authority into the browser. `SLICE-UI-001` (Product UI Foundation & Existing Screen Retrofit) is an **implementation-enablement slice** operating entirely within this existing Frontend Architecture baseline — it does not change system architecture, and its implementation-level governance (component architecture, design-token scope, external-component intake policy, and validation strategy) is recorded in `docs/implementation/UI_FOUNDATION_REGISTER.md`, which carries no Architecture authority of its own and must not be read as amending this document.
